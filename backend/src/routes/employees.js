...
function normalizeRole(r) {
  ...
  if ((s.replace(/[^a-z]/g, '').includes('storekeeper')) || (s.includes('store') && s.includes('keeper'))) return 'storeKeeper';
  return null;
}
...
router.get('/', authenticate, async (req, res) => {
  try {
    ...
    // If manager requester, exclude Owner and Manager roles from results
    if (requester.role === 'manager') {
      filter.role = { $nin: ['owner', 'manager'] };
    }

    const users = await User.find(filter).select('-passwordHash -__v').lean();
    // Ensure active flag present for all users (default to true)
    const normalized = users.map(u => ({ ...u, active: (u.active === undefined || u.active === null) ? true : u.active }));
    res.json(normalized);
  } catch (err) {
    console.error(err);
...
router.post('/', authenticate, async (req, res) => {
  try {
    ...
    const assignedMartId = requester.role === 'systemAdmin' ? (martId || null) : requester.martId;

    const user = new User({ name, username, passwordHash, role: normRole, martId: assignedMartId, phone, salary });
    await user.save();
...
router.put('/:id', authenticate, async (req, res) => {
  try {
    ...
    if (requester.role !== 'systemAdmin') {
      if (!requester.martId || String(requester.martId) !== String(target.martId)) return res.status(403).json({ message: 'Insufficient permissions' });
    }
    ...
    if (requester.role === 'manager' && update.role) {
      if (['manager','owner','systemAdmin'].includes(update.role)) return res.status(403).json({ message: 'Manager cannot set this role' });
    }

    const updated = await User.findByIdAndUpdate(id, update, { new: true }).select('-passwordHash -__v');
    res.json(updated);
  } catch (err) {
    console.error(err);
    res.status(500).json({ message: 'Server error' });
...
router.delete('/:id', authenticate, async (req, res) => {
  try {
    ...
    if (requester.role !== 'systemAdmin') {
      if (!requester.martId || String(requester.martId) !== String(target.martId)) return res.status(403).json({ message: 'Insufficient permissions' });
      if (requester.role !== 'owner') return res.status(403).json({ message: 'Only owners can delete employees' });
    }
