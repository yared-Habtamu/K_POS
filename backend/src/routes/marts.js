...
router.post('/register', async (req, res) => {
  try {
    ...
    ownerConfirmPassword,
      ...
      if (!martName || !ownerName || !ownerPhone || !ownerPassword || !ownerUsername || !ownerConfirmPassword) {
    ...
    // sanitize helper
    const sanitize = (s) => (s || '').toString().toLowerCase().replace(/[^a-z0-9]/g, '').slice(0, 30);

    let username = '';
    if (ownerUsername) {
      // owner provided a username — sanitize and ensure uniqueness
      const proposed = sanitize(ownerUsername);
      if (!proposed) return res.status(400).json({ message: 'Invalid owner username' });
    ...
    const owner = new User({ name: ownerName, username, phone: ownerPhone, passwordHash, role: 'owner' });
    await owner.save();
    ...
    if (err && err.code === 11000) {
      // duplicate key
      return res.status(409).json({ message: 'Duplicate resource', key: err.keyValue });
    }
...
router.get('/pending', async (req, res) => {
  try {
    const list = await Mart.find({ status: 'pending' }).populate('ownerId', 'name phone');
    res.json(list);
  } catch (err) {
    console.error(err);
    res.status(500).json({ message: 'Server error' });
...
router.put('/:id/approve', async (req, res) => {
  try {
    const { id } = req.params;
    const mart = await Mart.findByIdAndUpdate(id, { status: 'approved' }, { new: true });
...
router.put('/:id/disable', async (req, res) => {
  try {
    const { id } = req.params;
    const mart = await Mart.findByIdAndUpdate(id, { status: 'disabled' }, { new: true });
    if (!mart) return res.status(404).json({ message: 'Mart not found' });
    res.json(mart);
  } catch (err) {
    console.error(err);
...
router.put('/:id/reject', async (req, res) => {
  try {
    const { id } = req.params;
    const mart = await Mart.findByIdAndUpdate(id, { status: 'rejected' }, { new: true });
    if (!mart) return res.status(404).json({ message: 'Mart not found' });
    res.json(mart);
  } catch (err) {
    console.error(err);
