...
router.get('/daily', authenticate, async (req, res) => {
  try {
    ...
    const sales = await Sale.find({ martId: targetMartId, date: { $gte: start, $lte: end } }).lean();

    const totalSales = sales.reduce((s, x) => s + (x.total || 0), 0);
    const grossSales = sales.reduce((s, x) => s + (x.subtotal || 0), 0);
    const discountsTotal = sales.reduce((s, x) => s + ((x.discount && x.discount.amount) || 0), 0);
    ...
    for (const s of sales) {
      ...
      const cid = s.cashierId ? String(s.cashierId) : (s.cashierName || 'unknown');
      if (!salesByCashier[cid]) salesByCashier[cid] = { cashierId: cid, cashierName: s.cashierName || 'Unknown', sales: 0 };
    ...
    res.json({
      ...
      grossSales,
      discountsTotal,
      salesByPaymentMethod: Object.entries(salesByPaymentMethod).map(([method,total]) => ({ method, total })),
