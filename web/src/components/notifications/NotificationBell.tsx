...
export function NotificationBell() {
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent
        align="end"
        className="w-80 md:w-96 p-0 glass-strong border-border/50 shadow-xl overflow-hidden"
      >
        <div className="p-4 flex items-center justify-between bg-primary/5">
          <div className="flex items-center gap-2">
            ...
            {unreadCount > 0 && (
              <Badge
                variant="destructive"
                className="h-5 min-w-[20px] px-1 flex items-center justify-center text-[10px] font-bold rounded-full"
              >
                {unreadCount > 99 ? "99+" : unreadCount}
              </Badge>
            )}
